...
public class SellerWebControllerTest {
    ...
    public void setUp(){
        ...
        fakeSellerRegister = new SellerRegisterDTO(
            "Walter White", "WalterWhite@Cooking.com", "11901010101", Department.FOOD
        );
        fakeSellerUpdate = new SellerUpdateDTO(
            "Jesse Pinkman", "JessePinkman@Cooking.com", "11904040404", Department.TECHNOLOGY
        );
    ...
    public void testListSellersWithOneFilter() throws Exception{
        ...
        verify(sService, times(1))
            .getAllSorted("name", "asc", null, "Walter White", null, null);    }
    ...
    public void testListSellersWithAllFilters() throws Exception{
        ...
        verify(sService, times(1))
            .getAllSorted("name", "asc", "FOOD","Walter White","WalterWhite@Cooking.com", "11901010101");
    }

    @Test
    @WithMockUser
    @DisplayName("Test if no Seller is being returned on getAllSorted with a filter that don't match anything and default user")
    public void testListWithNoSellers() throws Exception{
        ...
        verify(sService, times(1))
            .getAllSorted("name", "asc", null, "Saul Goodman", null, null);
    }
    ...
    public void testSellerEditFormAsAdmin() throws Exception{
        ...
        verify(sService, times(1))
            .getById(fakeSeller1Response.id());
    }
    ...
    public void testSubmitNewSellerAsUser() throws Exception{
        mockMvc.perform(
            post("/seller/form")
                ...
                .param("mail", fakeSellerRegister.mail())
                .param("phone", fakeSellerRegister.phone())
                .param("department", fakeSellerRegister.department().name())
        ).andExpect(status().isForbidden());

        verify(sService, times(0))
            .register(fakeSellerRegister);
    }
    ...
    public void testSubmitNewSellerAsAdminWithInvalidMail() throws Exception{
        fakeSellerRegister = new SellerRegisterDTO(
            "Jesse Pinkman", "invalidMail", "11904040404", Department.TECHNOLOGY
        );
        fakeSeller = new Seller();

        when(sService.register(any(SellerRegisterDTO.class)))
            .thenThrow(new MultipleValidationException(List.of(new InvalidMailException("Invalid mail"))));
        when(sMapper.toEntity(fakeSellerRegister)).thenReturn(fakeSeller);
        ...
        .andExpect(view().name("seller-form"))
        .andExpect(model().attributeExists("seller"))
        .andExpect(model().attribute("seller", fakeSeller))
        .andExpect(model().attribute("departments", Department.values()))
    ...
    public void testSubmitNewSellerAsAdminWithInvalidPhone() throws Exception{
        fakeSellerRegister = new SellerRegisterDTO(
            "Jesse Pinkman", "JessePinkman@Cooking.com", "123", Department.TECHNOLOGY
        );
        fakeSeller = new Seller();
    ...
    public void testSubmitNewSellerAsAdminWithInvalidMailAndPhone() throws Exception{
        fakeSellerRegister = new SellerRegisterDTO(
        "Jesse Pinkman", "invalidmail", "123", Department.TECHNOLOGY
        );
        fakeSeller = new Seller();
    ...
    public void testSubmitEditSellerAsUser() throws Exception{
        mockMvc.perform(
            post("/seller/form/{id}", 1)
                .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .param("name", fakeSellerUpdate.name())
                .param("mail", fakeSellerUpdate.mail())
                .param("phone", fakeSellerUpdate.phone())
                .param("department", fakeSellerUpdate.department().name())
        ).andExpect(status().isForbidden());

        verify(sService, times(0))
            .update(1, fakeSellerUpdate);
    }
    ...
    public void testSubmitEditSellerAsAdminWithInvalidMail() throws Exception{
        ...
        when(sService.update(eq(1), any(SellerUpdateDTO.class)))
            .thenThrow(new MultipleValidationException(List.of(new InvalidMailException("Invalid mail"))));
        when(sMapper.toEntity(fakeSellerUpdate)).thenReturn(fakeSeller);
