...
public class SupplyWebControllerTest {
    ...
    public void testListAllSupply() throws Exception{
        ...
        verify(sService, times(1))
            .getAllSorted("productId", "asc", null);
    ...
    public void testListSupplyWithOneFilter() throws Exception{
        when(sService.getAllSorted("productId", "asc", 1))
            .thenReturn(List.of(fakeSupply1Response));
        when(pService.getAll())
            .thenReturn(List.of(fakeProduct1Response, fakeProduct2Response));
    ...
    public void testSupplyFormAsAdmin() throws Exception{
        when(pService.getAllNotRegisteredOnSupply())
            .thenReturn(List.of(fakeProduct1Response, fakeProduct2Response));
    ...
    public void testSupplyEditFormAsAdmin() throws Exception{
        ...
        when(pService.getAllNotRegisteredOnSupply(fakeSupply1Response.id())).thenReturn(List.of(fakeProduct2Response));
        ...
        .andExpect(model().attribute("page", "supply"))
        .andExpect(model().attribute("products", List.of(fakeProduct2Response)));
    ...
    public void testSubmitNewSupplyAsUser() throws Exception{
        mockMvc.perform(
            post("/supply/form")
                ...
                .param("productId", fakeSupplyRegister.productId().toString())
        ).andExpect(status().isForbidden());

        verify(sService, times(0))
            .register(fakeSupplyRegister);
        verify(sMapper, times(0))
            .toEntity(fakeSupplyRegister);
        verify(pService, times(0))
            .getAll();
    }
    ...
    public void testSubmitNewSupplyAsAdminWithProductAlreadyRegistered() throws Exception{
        when(sService.register(any(SupplyRegisterDTO.class)))
            .thenThrow(new ProductRegisteredException("This product is already registered on supply, please update it instead"));
        when(sMapper.toEntity(fakeSupplyRegister)).thenReturn(fakeSupply);
        when(pService.getAll()).thenReturn(List.of(fakeProduct1Response, fakeProduct2Response));
    ...
    public void testSubmitEditSupplyAsUser() throws Exception{
        mockMvc.perform(
            post("/supply/form/{id}", 1)
                .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .param("quantity", fakeSupplyUpdate.quantity().toString())
                .param("productId", fakeSupplyUpdate.productId().toString())
        ).andExpect(status().isForbidden());

        verify(sService, times(0))
            .update(1, fakeSupplyUpdate);
    }
